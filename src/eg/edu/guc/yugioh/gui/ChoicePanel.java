...
public class ChoicePanel extends JPanel {
	...
	public void update(ArrayList<Card> m){
		...
		for (int i=0;i<m.size();i++) {
			if (m.get(i) instanceof MonsterCard) {
		...
		for(int i = 0; i <hand.size();i++){
			if(hand.get(i) instanceof MonsterCard){
				...
				ImageIcon img = new ImageIcon("Cards Images Database/Monsters/"+hand.get(i).getName()+".jpg");
				...
				ImageIcon img = new ImageIcon("Cards Images Database/Spells/"+hand.get(i).getName()+".jpg");
				Image img2 = img.getImage();
				Image newimg = img2.getScaledInstance(100, 146,  java.awt.Image.SCALE_SMOOTH);
				ImageIcon newIcon = new ImageIcon(newimg);
